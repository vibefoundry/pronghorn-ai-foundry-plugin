# VibeFoundryAI plugin for Claude Code and Codex

This is the distribution repo for the VibeFoundryAI plugin, VibeFoundry's edition of the VibeFoundry toolkit. It holds only what your machine needs to connect: the plugin manifests, the server addresses, and a small launcher for the local file viewer. Everything else runs on VibeFoundry's servers. One repo serves both hosts: Claude reads `.claude-plugin/`, Codex reads `.agents/plugins/` and `.codex-plugin/`.

## Install in Codex

In the Codex app open Plugins, choose to add a marketplace, and paste
`vibefoundry/vibefoundry-ai-plugin` (or the full URL below). Then install
**VibeFoundryAI** from the `vibefoundry-ai` marketplace. From a terminal it is:

```bash
codex plugin marketplace add https://github.com/vibefoundry/vibefoundry-ai-plugin.git
```

```bash
codex plugin add vibefoundry-ai@vibefoundry-ai
```
...
```bash
claude plugin marketplace add https://github.com/vibefoundry/vibefoundry-ai-plugin.git
```

```bash
claude plugin install vibefoundry-ai@vibefoundry-ai
```
...
```bash
claude plugin marketplace update vibefoundry-ai && claude plugin update vibefoundry-ai@vibefoundry-ai
```

## What is in here

- `plugins/vibefoundry-ai/.mcp.json` - the hosted toolkit server and the local pane server (Claude); `.mcp.codex.json` is the same pair in Codex's shape.
- `plugins/vibefoundry-ai/.codex-plugin/plugin.json` and `.agents/plugins/marketplace.json` - the Codex manifests.
- `plugins/vibefoundry-ai/hooks/hooks.json` - opens the pane at the start of every session.
...
- `plugins/vibefoundry-ai/server/pane.py` - starts and stops the file viewer on your machine. It fetches the viewer from the hosted server and runs it on loopback. Your files never leave your computer.
